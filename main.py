...
async def cmd_start(message: Message):
    await message.answer(
        ...
        f'/convert - выбрать валюту и конвертировать ее.\n'
...
async def cmd_help(message: Message):
    await message.answer(
        f'Чем могу помочь, {message.from_user.first_name}? Привет! Я бот, который поможет тебе конвертировать валюты быстро и без лишних усилий. Вот список доступных команд:\n '
...
async def cmd_convert(message: Message):
    ...
    if len(user_input) != 4:
        await message.answer(
            'Неверный формат команды! Пожалуйста, используйте: /convert <сумма> <из_валюты> <в_валюту>')
        return
...
async def handle_text(message: Message):
    logger.info(f"Пользователь {message.from_user.full_name} отправил сообщение: {message.text}")
